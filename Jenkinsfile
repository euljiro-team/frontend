...
//                sh "docker pull voyagerss_vuejs_develop"
                ...
                sh "docker rm -f ${env.JOB_NAME}"
                sh "docker run --name ${env.JOB_NAME}_${git_branch} -it -d -p 80:80 -p 443:443 -v /mnt/c/logs:/var/log/nginx ${env.JOB_NAME}_${git_branch}:${BUILD_NUMBER}"
//                docker run --name voyagerss_vuejs_develop -it -d -p 80:80 -p 443:443 -v /mnt/c/logs:/var/log/nginx voyagerss_vuejs_develop
            } catch (e) {
